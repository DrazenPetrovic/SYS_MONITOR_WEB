...
import { Router, AlertCircle, Wifi, WifiOff, ShieldCheck, ShieldOff } from 'lucide-react';
  ...
  if (router._error) {
    return (
      <div className="flex items-center gap-2 text-red-400 text-xs bg-red-400/10 border border-red-400/20 rounded-lg p-3 mt-3">
        ...
        <span>Nedostupan — {router.details || 'provjeri konekciju i credentials'}</span>
      </div>
    );
  }

  const { resource = {}, interfaces = [], connectionCount = 0, l2tpClients = [], pppActive = [] } = router;
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mb-3">
        ...
        {resource['cpu-count'] && <span>{resource['cpu-count']} CPU · {resource.architecture}</span>}
        {resource.uptime && (
          <span>
            uptime <span className="text-slate-300 font-mono font-medium">{resource.uptime}</span>
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <div className="bg-slate-800/50 rounded-lg p-3">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xl font-bold font-mono tabular-nums" style={{ color: statusColor(cpuLoad) }}>
              {cpuLoad}%
            </span>
          </div>
          <div className="h-2 bg-slate-700/60 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width: `${cpuLoad}%`, backgroundColor: statusColor(cpuLoad) }} />
          </div>
          <div className="text-xs text-slate-600 mt-1.5">{resource['cpu-frequency']} MHz</div>
        </div>

        <div className="bg-slate-800/50 rounded-lg p-3">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-slate-400">Memory</span>
            <span className="text-xl font-bold font-mono tabular-nums" style={{ color: statusColor(memPct) }}>
              {memPct.toFixed(0)}%
            </span>
          </div>
          <div className="h-2 bg-slate-700/60 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width: `${memPct}%`, backgroundColor: statusColor(memPct) }} />
          </div>
          <div className="text-xs text-slate-600 mt-1.5">{fmtBytes(usedMem)} / {fmtBytes(totalMem)}</div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-lg p-3">
          ...
          <div className="h-2 bg-slate-700/60 rounded-full overflow-hidden mb-1.5">
            <div className="h-full rounded-full bg-slate-500 transition-all duration-700"
              style={{ width: `${hddPct}%` }} />
          </div>
          <div className="flex justify-between text-xs text-slate-600">
            <span>{fmtBytes(totalHdd - freeHdd)} used</span>
            <span className="text-cyan-500 font-medium">{connectionCount} conn.</span>
          </div>
        </div>
      </div>
      ...
      {(l2tpClients.length > 0 || pppActive.length > 0) && (
        <div className="mb-4">
          ...
          <div className="space-y-1.5">
                ...
                <div key={tunnel.name}
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-mono text-slate-200 font-medium">{tunnel.name}</div>
                      <div className="text-slate-500 truncate">→ {tunnel['connect-to']} · user: {tunnel.user}</div>
                    </div>
                  </div>
                  <div className="text-right shrink-0 ml-3">
                    ...
                    {tunnel.uptime && (
                      <div className="text-slate-600 font-mono">{tunnel.uptime}</div>
                    )}
                  </div>
                </div>
              );
            ...
            {pppActive.map((session, i) => (
              <div key={i}
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="font-mono text-slate-200 font-medium">{session.name}</div>
                ...
                <div className="text-right shrink-0 ml-3">
                  ...
                  {session.uptime && (
                    <div className="text-slate-600 font-mono">{session.uptime}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            {l2tpClients.length === 0 && pppActive.length === 0 && (
              <div className="text-slate-600 text-xs py-2 text-center">Nema aktivnih VPN sesija</div>
            )}
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-slate-500">
            ...
            <span className="text-slate-600 ml-1">
              ({interfaces.filter(i => i.running === 'true').length} active / {interfaces.length})
            </span>
          </span>
          {interfaces.length > 6 && (
          ...
          return (
            <div key={iface.name}
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="font-mono text-slate-200 truncate">{iface.name}</span>
                {iface.comment && (
                  <span className="text-slate-600 truncate hidden sm:inline">{iface.comment}</span>
                )}
              </div>
              <div className="flex gap-3 font-mono shrink-0 ml-2">
                {isUp && (iface.rxRate > 0 || iface.txRate > 0) ? (
                  <>
                    <span className="text-cyan-400">↓ {fmtSpeed(iface.rxRate)}</span>
                    <span className="text-orange-400">↑ {fmtSpeed(iface.txRate)}</span>
                  </>
                ) : (
                  <>
                    <span className="text-slate-600">↓ {fmtBytes(iface['rx-byte'])}</span>
                    <span className="text-slate-600">↑ {fmtBytes(iface['tx-byte'])}</span>
                  </>
                )}
              </div>
            </div>
...
export default function MikrotikCard({ data }) {
